...
static long xdl_split(xdfile_t *xdf1, long off1, long lim1,
	...
	for (ec = 1;; ec++) {
		...
		for (d = fmax; d >= fmin; d -= 2) {
			...
			prev1 = i1;
			i2 = i1 - d;
			for (; i1 < lim1 && i2 < lim2 && get_mph(xdf1, i1) == get_mph(xdf2, i2); i1++, i2++);
		...
		for (d = bmax; d >= bmin; d -= 2) {
			...
			prev1 = i1;
			i2 = i1 - d;
			for (; i1 > off1 && i2 > off2 && get_mph(xdf1, i1 - 1) == get_mph(xdf2, i2 - 1); i1--, i2--);
		...
		if (got_snake && ec > xenv->heur_min) {
			for (best = 0, d = fmax; d >= fmin; d -= 2) {
				...
				if (v > XDL_K_HEUR * ec && v > best &&
					...
					for (k = 1; get_mph(xdf1, i1 - k) == get_mph(xdf2, i2 - k); k++)
						if (k == xenv->snake_cnt) {
							best = v;
							spl->i1 = i1;
							spl->i2 = i2;
			...
			for (best = 0, d = bmax; d >= bmin; d -= 2) {
				...
				if (v > XDL_K_HEUR * ec && v > best &&
					...
					for (k = 0; get_mph(xdf1, i1 + k) == get_mph(xdf2, i2 + k); k++)
...
int xdl_recs_cmp(xdfile_t *xdf1, long off1, long lim1,
	 ...
	 */
	for (; off1 < lim1 && off2 < lim2 && get_mph(xdf1, off1) == get_mph(xdf2, off2); off1++, off2++);
	for (; off1 < lim1 && off2 < lim2 && get_mph(xdf1, lim1 - 1) == get_mph(xdf2, lim2 - 1); lim1--, lim2--);
...
int xdl_do_diff(xdfile_t *xdf1, xdfile_t *xdf2, xpparam_t const *xpp,
		usize mph_size, xdfenv_t *xe) {
	long ndiags;
	long *kvd, *kvdf, *kvdb;
	xdalgoenv_t xenv;
	int res;

	if (xdl_env_prepare(xdf1, xdf2, xpp->flags, mph_size, xe) < 0)
		return -1;
			  ...
			  struct split_measurement *m)
{
	...
	m->post_blank = 0;
	m->post_indent = -1;
	for (i = split + 1; i < xdf->record.length; i++) {
...
int xdl_diff(mmfile_t *mf1, mmfile_t *mf2, xpparam_t const *xpp,
	...
	xdl_file_prepare(mf1, xpp->flags, &xdf1);
	xdl_file_prepare(mf2, xpp->flags, &xdf2);
