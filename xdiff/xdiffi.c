...
static int group_slide_down(struct xd_file_context *ctx, struct xdlgroup *g) {
	u64 mph1 = ctx->minimal_perfect_hash->ptr[g->start];
	u64 mph2 = ctx->minimal_perfect_hash->ptr[g->end];

	if (g->end < ctx->record->length && mph1 == mph2) {
...
static int group_slide_up(struct xd_file_context *ctx, struct xdlgroup *g) {
	u64 mph1 = ctx->minimal_perfect_hash->ptr[g->start - 1];
	u64 mph2 = ctx->minimal_perfect_hash->ptr[g->end - 1];
...
static void xdl_mark_ignorable_lines(xdchange_t *xscr, struct xdpair *pair, long flags)
{
	...
	for (xch = xscr; xch; xch = xch->next) {
		...
		for (i = 0; i < xch->chg1 && ignore; i++)
			ignore = xdl_blankline(rec[i].ptr, rec[i].size_with_eol, flags);
